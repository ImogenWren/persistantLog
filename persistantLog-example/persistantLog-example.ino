...
persistantLog dataLog;

void setup() {
  Serial.begin(115200);
  delay(2500);
  Serial.println("\n\nTesting Basic EEPROM Like Wear Testing Test\n");
  dataLog.check_eeprom_length();
  dataLog.struct_size_check();
  delay(2000);
  logData_t recalled_data = dataLog.get_latest_log();
  Serial.println("Printing Recalled Stats: ");
  dataLog.print_stats(recalled_data);
  delay(2000);
  ...
  dataLog.write_log(recalled_data);
  delay(2000);
  Serial.println("Program Finished Writing Changes");
  recalled_data = dataLog.get_latest_log();
